...
SCRIPT_VERSION = "texture-test-v7-batch50-48view-overlapping-rings-v5-texture"
...
def _render_textured_dataset_impl(
    *,
    ...
    if diagnostics["foreground_gray_std_min"] < 5.0:
        raise RuntimeError(
    ...
    *,
    ...
    if model_dir.exists() and force:
        ...
        if diagnostics_path.is_file():
            try:
                existing = json.loads(diagnostics_path.read_text(encoding="utf-8"))
                previous_status = existing.get("status")
                existing["status"] = "skipped_existing"
                existing["resume_previous_status"] = previous_status
                existing["final_mesh_vertices"] = final_mesh_vertices
                existing["final_mesh_faces"] = final_mesh_faces
                existing["coarse_obj"] = str(coarse_obj)
                return existing
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Batch Sofa50 coarse acquisition: frozen 48-view overlapping-ring render with V5 vertex texture -> "
            "COLMAP known-pose triangulation -> OpenMVS densification/mesh, with resume and per-model failure isolation. "
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Optional Sofa model ID. Repeat for a subset. If omitted, processes all IDs "
            "from selection.json (expected Sofa50 count: 50)."
        ),
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Explicitly request all IDs in Sofa50 selection.json; batch mode already does this when --model-id is omitted.",
    )
    parser.add_argument(
        "--expected-count",
        type=int,
        default=50,
        help="Expected number of models when running the full selection (default: 50).",
    )
    parser.add_argument(
        "--keep-incomplete",
        action="store_true",
        help="Do not automatically delete incomplete per-model render/reconstruction directories on resume.",
    )
    parser.add_argument(
        "--image-size",
        type=int,
    ...
    run_full_selection = bool(args.all or not args.model_id)
    model_ids = resolve_sofa_model_ids(sofa_root, args.model_id, run_full_selection)
    if run_full_selection and args.expected_count > 0 and len(model_ids) != args.expected_count:
        raise RuntimeError(
            f"Expected {args.expected_count} Sofa50 IDs in selection.json, found {len(model_ids)}. "
            "Use --expected-count 0 to disable this check."
        )
    print(f"Processing {len(model_ids)} textured OpenMVS model(s): {model_ids}", flush=True)
    print(
    ...
    def write_checkpoint_summary() -> None:
        ...
        payload = {
            "script_version": SCRIPT_VERSION,
            "experiment": "sofa50_batch50_48view_overlapping_rings_smoothed_random_vertex_texture_openmvs",
    ...
    for index, model_id in enumerate(model_ids, start=1):
        ...
        # Resume semantics: successful coarse.obj is immutable and reused. A stale
        # partial directory from a killed/failed previous run is rebuilt automatically.
        if not args.force and not args.keep_incomplete:
            render_complete = (render_dir / "dataset.json").is_file() and (
                render_dir / "texture_render_diagnostics.json"
            ).is_file()
            if render_dir.exists() and not render_complete:
                print(f"[resume cleanup] removing incomplete render: {render_dir}", flush=True)
                shutil.rmtree(render_dir)
            if model_dir.exists() and not coarse_obj.is_file():
                print(f"[resume cleanup] removing incomplete reconstruction: {model_dir}", flush=True)
                shutil.rmtree(model_dir)
        ...
        except Exception as exc:  # noqa: BLE001 - batch must continue after one model fails
            failure = {
                "script_version": SCRIPT_VERSION,
                "model_id": model_id,
                "status": "failed",
                "source_sofa_mesh": str(mesh_path),
                "error_type": type(exc).__name__,
                "error": str(exc),
                "traceback": traceback.format_exc(),
            }
            results.append(failure)
            failure_path = output_root / "models" / model_id / "batch_failure.json"
            failure_path.parent.mkdir(parents=True, exist_ok=True)
            failure_path.write_text(json.dumps(failure, indent=2) + "\n", encoding="utf-8")
            print(
                f"[FAILED {index}/{len(model_ids)}] {model_id}: {type(exc).__name__}: {exc}",
                file=sys.stderr,
                flush=True,
            )
        finally:
    ...
    write_checkpoint_summary()
    failed_ids = [str(item.get("model_id")) for item in results if item.get("status") == "failed"]
    usable = sum(
        1 for item in results if item.get("status") in {"ok", "skipped_existing"}
    )
    print(f"\nBatch complete: usable={usable}/{len(model_ids)}, failed={len(failed_ids)}", flush=True)
    if failed_ids:
        print(f"Failed model IDs: {failed_ids}", flush=True)
    print(f"Wrote incremental summary: {summary_path}", flush=True)
