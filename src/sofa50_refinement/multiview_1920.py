...
def prepare_multiview_dataset(
    ...
    *,
    ...
    full_forward_validation: bool = False,
    workers: int = 1,
) -> dict[str, Any]:
    ...
    output_root = Path(output_root or refinement_root / "multiview_1920").expanduser().resolve()
...
def _prepare_one(
    ...
    *,
    ...
    if force or not all(path.is_file() for path in required):
        ...
        depth_paths = [render_dir / "depth" / f"{view:04d}.npy" for view in range(RENDER_SPEC["views"])]
        ...
        source["source_image_size"] = [
            RENDER_SPEC["image_size"], RENDER_SPEC["image_size"]
        ]
        source["prepared_image_size"] = RENDER_SPEC["image_size"]
...
def _expanded_inference_sample(
    ...
    sample = {
        ...
        "source_image_size": [RENDER_SPEC["image_size"], RENDER_SPEC["image_size"]],
        "prepared_image_size": RENDER_SPEC["image_size"],
...
def _audit_one(
    ...
    gt_mesh = deps["load_mesh"](gt_path).ensure_normals()
    expanded_mesh = deps["load_mesh"](expanded_path).ensure_normals()
    if len(dataset.image_paths) != RENDER_SPEC["views"] or len(dataset.cameras) != RENDER_SPEC["views"]:
        raise ValueError(
            f"{model_id}: downstream dataset does not contain "
            f"{RENDER_SPEC['views']} views"
        )
    rgb_shapes, mask_shapes, depth_shapes = [], [], []
    finite_depth_inside = []
    background_depth_inf = []
    for view in range(RENDER_SPEC["views"]):
    ...
    image_size = RENDER_SPEC["image_size"]
    if set(map(tuple, rgb_shapes)) != {(image_size, image_size, 3)}:
        raise ValueError(
            f"{model_id}: RGB output is not uniformly "
            f"[{image_size},{image_size},3]"
        )
    if set(map(tuple, mask_shapes)) != {(image_size, image_size)} or set(
        map(tuple, depth_shapes)
    ...
    expected_visibility = {
        "gt_mask_support": [RENDER_SPEC["views"], gt_mesh.num_vertices],
        "gt_depth_visible": [RENDER_SPEC["views"], gt_mesh.num_vertices],
        "expanded_mask_support": [RENDER_SPEC["views"], expanded_mesh.num_vertices],
        "expanded_depth_visible": [RENDER_SPEC["views"], expanded_mesh.num_vertices],
    }
    ...
    return {
        ...
        "rgb_shape_per_view": [image_size, image_size, 3],
        "mask_shape_per_view": [image_size, image_size],
        "depth_shape_per_view": [image_size, image_size],
...
def _validate_downstream_loaders(
    ...
    *,
    ...
    # This config filename is legacy-named "960", but the config itself does not
    # contain an image-resolution field. Image tensors keep the resolution stored in
    # the prepared sample, so it can be reused for the 1920 interface/shape audit.
    config = _read_json(
    ...
    # 1920 images are much larger than the original 960 inputs. By default this
    # validation stays static-only so dataset generation does not spend large amounts
    # of RAM/VRAM on model forwards. Set full_forward_validation=True explicitly if
    # you want to decode and forward every prepared sample.
    for split in present_splits:
        ...
        for index, model_id in enumerate(gt_dataset.sample_ids):
            ...
            run_forward = full_forward_validation
            gt_image_shape = [RENDER_SPEC["views"], 3, RENDER_SPEC["image_size"], RENDER_SPEC["image_size"]]
            if run_forward:
                ...
                if tuple(gt_image_shape) != (RENDER_SPEC["views"], 3, RENDER_SPEC["image_size"], RENDER_SPEC["image_size"]):
            ...
            expanded_image_shape = [RENDER_SPEC["views"], 3, RENDER_SPEC["image_size"], RENDER_SPEC["image_size"]]
            ...
            if run_forward:
                ...
                if tuple(expanded_image_shape) != (RENDER_SPEC["views"], 3, RENDER_SPEC["image_size"], RENDER_SPEC["image_size"]):
...
def _report(audit: dict[str, Any], observations_path: Path) -> str:
    ...
    lines = [
        ...
        f"- RGB: {RENDER_SPEC['views']} x `[{RENDER_SPEC['image_size']}, {RENDER_SPEC['image_size']}, 3]` PNG; "
        f"mask: {RENDER_SPEC['views']} x `[{RENDER_SPEC['image_size']}, {RENDER_SPEC['image_size']}]` PNG; "
        f"depth: {RENDER_SPEC['views']} x `[{RENDER_SPEC['image_size']}, {RENDER_SPEC['image_size']}]` NPY.",
        f"- Cameras: intrinsics `[{RENDER_SPEC['views']},3,3]`, "
        f"extrinsics `[{RENDER_SPEC['views']},4,4]`, right-handed CV world-to-camera.",
